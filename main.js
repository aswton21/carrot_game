...
const audio = new Audio("carrot/sound/bg.mp3");
let id = 0;
let cnt = 10;
let sec = 10;
...
function generateCarrots() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    carrot_img.src = "carrot/img/carrot.png";
...
function generateBugs() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    bug_img.src = "carrot/img/bug.png";
...
gameArea.addEventListener("click", () => {
  ...
  if (game_status) {
    if (event.target.className === "bug") {
      const audio_bug = new Audio("carrot/sound/bug_pull.mp3");
      audio_bug.play();
      lost_end();
    }
  }
